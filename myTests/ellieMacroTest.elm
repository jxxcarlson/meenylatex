module EllieMacroTest exposing (main)
...
main =
    Browser.staticPage
        (pre []
            [ line <| "SSCCE"
            , debug "macroArg" (run macroArgWord "foo")
            , debug "macroArgWords" (run macroArgWords "foo bar baz")
            , debug "macro" <| (run macroName) "\\wasup"
            , debug "macro" <| (run macro) "\\wasup{foo}{bar}"
            , debug "macro" <| (run macro) "\\wasup{\\lol{foo}}{bar, baz}"
