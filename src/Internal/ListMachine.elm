...
sends the interenal state to `before + current + after`:
    ...
    sumState internalState =
        let
            a =
                internalState.before |> Maybe.withDefault 0

            b =
                internalState.current |> Maybe.withDefault 0

            c =
                internalState.after |> Maybe.withDefault 0
        in
        a + b + c
...
-}
type alias State a =
    { before : Maybe a, current : Maybe a, after : Maybe a, inputList : List a }


type alias TotalState a b =
    { state : State a, outputList : List b }


type alias Reducer a b =
    a -> TotalState a b -> TotalState a b
...
run_ reducer inputList =
    let
        ...
        finalTotalState =
            makeMachine reducer initialTotalState_ inputList
    in
...
nextState internalState_ =
    let
        nextInputList_ =
            List.drop 1 internalState_.inputList
    in
...
makeReducer computeOutput input machineState =
    let
        nextInputList =
            List.drop 1 machineState.state.inputList

        nextInternalState_ =
            nextState machineState.state

        newOutput =
            computeOutput machineState.state

        outputList =
            newOutput :: machineState.outputList
    in
